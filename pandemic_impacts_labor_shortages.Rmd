...
Those numbers have decreased since October, largely due to the seasonality of employment in Greater Minnesota. 
...
Comparing October vs November a few occupational groups see an increase in the number of EDRs in which they are facing workforce shortages.

* Management - from 1 to 6 EDRs
* Healthcare practitioners and technical - from 12 to 13 EDRs
* Community and social services - from 11 to 13 EDRs
* Computer and mathematical - from 7 to 10 EDRs
* Architecture and engineering - from 8 to 9 EDRs
* Business and financial operations - from 2 to 7 EDRs
* Management - from 1 to 6 EDRs
* Personal care and service - from 3 to 6 EDRs
* Protective service - from 3 to 6 EDRs
* Arts, design, entertainment, sports, and media - from 2 to 5 EDRs
* Healthcare support - from 0 to 5 EDRs
* Installation, maintenance and repair - from 2 to 5 EDRs
* Office and administrative support - from 1 to 5 EDRs
* Legal - from 3 to 4 EDRs

Here are the occupational groups that dropped in the number of EDRs in which they are experiencing workforce shortages
* Transportation and material moving - from 4 to 1 EDR
* Buildings and grounds cleaning and maintenance - from 1 to 0 EDRs
...
ui.job.postings.novavg.edr <- ui.job.postings.weekly.occ.edr %>%
  filter(ui.month == "Nov",
         `Dim Title` != "unknown") %>%
  group_by(edr, `Dim Title`) %>%
  summarise(avg.job.posting.ui.difference = round(mean(job.posting.ui.difference))) %>%
...
labor.shortage.novavg.occ.num.edr <- ui.job.postings.novavg.edr %>%
  filter(shortage.surplus == "Shortage") %>%
  group_by(`Dim Title`) %>%
  distinct(edr) %>%
  summarise(num.edrs = n()) %>%
...
labor.surplus.novavg.occ.num.edr <- ui.job.postings.novavg.edr %>%
  filter(shortage.surplus == "Surplus") %>%
  group_by(`Dim Title`) %>%
  distinct(edr) %>%
  summarise(num.edrs = n()) %>%
...
## October occupational groups - shortages


```{r table occupational groups shortages october}
...
## November occupational groups - shortages

```{r table occupational groups shortages november}

kable(format = "html", labor.shortage.novavg.occ.num.edr, escape = F, align = "c", caption = "Number of EDRs where an occupation group was facing a workforce shortage using the average difference between job postings and unemployment claims through the month of November.") %>%
  kable_styling(bootstrap_options = "striped", full_width = TRUE) %>%
  scroll_box(width = "100%", height = "100%")
...
# Specific occupations within occupational groups with shortages{.tabset}
