...
* Community and social services (in top-5 for 10 of 13 EDRs): Nearly all of the positions available were categorized as social and human service assistant or mental health and substance abuse counselors.

* Transportation and material moving (in top-5 for 8 of 13 EDRs): Most of the positions were concentrated in heavy and tracter-trailer truck driver, light truck or delivery services, drivers or sales workers, as well as laborers and freight, stock, and material movers. 

* Office and administrative support (in top-5 for 5 of 13 EDRs): the occupations available within this field were concentrated in stock clerks and order fillers, first-line supervisors of office and administrative office workers, and customer sales representative.

* Production (in top-5 for 5 of 13 EDRs): For regions with production as one of their top in-demand jobs, most of the positions were concentrated in occupations such as assemblers and fabricators, first-line supervisors, as well as welders, cutters, solderers, and brazers.

* Architecture and engineering (in top-5 of 4 of 13 EDRs) : A majority of these positions were classified as industrial or mechanical engineers. 

* Computer and mathematical (in top-5 of 3 of 13 EDRs): A majority of these occupations were classified as "computer occupations" or "software developers". 

* Life, physical, and social science (in top-5 in 3 of 13 EDRs): the occupations available within this field was dependent on location. In our northern regions, the occupations were primarily revolve around environmental sciences and protection otherwise the majority of jobs were in the medical scientists occupations.

* Personal care and service (in top-5 in 3 of 13 EDRs): most of the positions were concentrated in the personal care aides occupations.

* Farming, fishing and forestry (in top-5 in 2 of 13 EDRs): these occupations were largely in ag related occupations  such as farmworkers and laborers, as well as agricultural equipment operators.

* Management (in top-5 in 2 of 13 EDRs): the occupations listed for these positions were broadly distributed across managers, marketing managers, general and operations managers, sales managers, and medical and health services managers. 

* Protective Service (in top-5 in 1 of 13 EDRs): a majority of the jobs in this field were security guards. 

* Installation, maintenance, and repair (in top-5 in 1 of 13 EDRs): A majority of the jobs in this occupation group were classified as "maintenance and repair workers" or "first-line supervisors of mechanics, installers and repairers".

* Sales and related (in top-5 in 1 of 13 EDRs): most of the positions were concentrated in the first-line supervisors of retail sales workers, retail salespersons and cashiers occupations.

* Business and financial operations (in top-5 in 1 of 13 EDRs): the majority of occupations in this field were classified as "management analysts" or "business operations specialists". 

* Healthcare support (in top-5 in 1 of 13 EDRs): nearly all of the positions available within this field were in the home health aides and nursing assistants occupations.
...
labor.shortages.top5.occ.list.edr <- labor.shortage.edr %>%
  filter(ui.date == max(labor.shortage.edr$ui.date),
         job.posting.ui.difference > 0) %>%
  group_by(edr) %>%
  arrange(desc(job.posting.ui.difference)) %>%
...
labor.shortage.soc.top5.edr <- job.postings.soc.sepoctavg.edr %>%
  group_by(edr, Job_Family) %>%
...
# Occupations with workforce shortages vs surpluses
...
labor.surplus.octavg.edr <- labor.surplus.edr %>%
  filter(ui.month == "Oct") %>%
  group_by(edr, planning.region, `Dim Title`) %>%
  summarise(avg.surplus = round(mean(job.posting.ui.difference))) %>%
  arrange(avg.surplus) %>%
  mutate(rank = seq(n()),
         avg.surplus = avg.surplus * -1) %>%
  ungroup() %>%
  filter(rank < 11)

labor.surplus.table <- labor.surplus.octavg.edr %>%
  mutate(label = paste(`Dim Title`, ": ", avg.surplus, sep = "")) %>%
  select(1,5,6)

labor.shortage.octavg.edr <- labor.shortage.edr %>%
  filter(ui.month == "Oct") %>%
  group_by(edr, planning.region, `Dim Title`) %>%
  summarise(avg.shortage = round(mean(job.posting.ui.difference))) %>%
  arrange(desc(avg.shortage)) %>%
  mutate(rank = seq(n()),
         avg.shortage = avg.shortage * -1) %>%
  ungroup()  %>%
  filter(rank < 11)

labor.shortage.table <- labor.shortage.octavg.edr %>%
  mutate(label = paste(`Dim Title`, ": ", avg.shortage, sep = "")) %>%
  select(1,5,6)
