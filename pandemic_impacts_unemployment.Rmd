...
Updated with November numbers: The last tab below provides a closeup of unemployment claims as a percentage of 2019 employment from August through the end of November. Overall, from their October lows, they are about 1 percentage point higher across most EDRs. However, they are significantly higher in some areas. EDRs in Northwest planning region have seen a significant increase since October such as EDR 5 - North Central which went from 4.2% to 7.5% by the end of November. EDR 7E - Central has also seen a significant increase in their unemployment claims since October,  going from 7% to 10% by December. Other EDRs in Southwest, Southeast, and the Seven County Metro have continued to be relatively flat since their lows in October.
...
## EDR - Last half of 2020

<br>

```{r chart ui claims as % of employment last half of 2020}
ui.pct.emp.edr.last.half.plot <- ggplot(filter(ui.oes.edr, date > "2020-07-31"), aes(date, ui.pct.empCount, color = edr)) +
  facet_wrap(~planning.region, ncol = 2)+
  geom_hline(yintercept = 0, color = "black") +
  geom_smooth(se = FALSE, size = 3) +
  geom_point_interactive(size = 5, aes(data_id = ui.pct.empCount, tooltip = paste(edr, "\nDate: ", date, "\nTotal employment in 2020: ", comma(EmpCount), "\nTotal unemployment continued claims: ", comma(ui.claims), "\nUnemployment claims as pct of total employment: ", percent(ui.pct.empCount), sep = ""))) +
...
girafe(ggobj = ui.pct.emp.edr.last.half.plot, height_svg = 10, width_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
```
...
The second tab below actually provides the difference in unemployment rates (in percentage points) between 2019 and 2020. What's interesting is to see that for the most part, the largest difference in the unemployment rate between 2019 and 2020was in the seven county metro. By May of 2020, the unemployment rate was 7.5 percentage points higher than in that same month in 2019. Even in November, where areas in Greater Minnesota have experienced an significant increase in unemployment claims are not that far off from 2019. For the most part, they are about 1 percentage point higher in November 2020 compared to 2019. Again, the highest difference continues to be the seven county metro and some of the EDRs in Central Minnesota. The rates in Southwest and Northwest are pretty comparable to 2019.

<br>

## Unemployment rates 2019 and 2020 comparison
...
unemp.rate.difference.edr <- master.unemp.rate.edr %>%
  mutate(year = year(`Year/Month`),
         month = month(`Year/Month`, label = TRUE),
         unemp.rate = unemp.rate * 100) %>%
  group_by(edr, month) %>%
  summarise(difference = unemp.rate[year == 2020] - unemp.rate[year == 2019]) %>%
  ungroup() %>%
...
## Unemployment rates 2019 and 2020 difference

<br>

```{r chart 2019 and 2020 aug through dec unemployment comparison}
unemp.rate.difference.edr.plot <- ggplot(unemp.rate.difference.edr , aes(as.factor(month), difference, color = edr, group = edr)) +
  geom_smooth(se = FALSE, size = 3) +
  geom_hline(yintercept = 0, color = "black") +
  geom_point_interactive(size = 5, aes(data_id = data_id, tooltip = paste(edr, "\n", month, "\nDifferece in Unemployment rate between 2020 and 2019: ", comma(difference, accuracy = .01), " percentage points", sep = ""))) +
  labs(x="", y = "", color="", title = "Difference in unemployment rate 2020 vs 2019")+
...
girafe(ggobj = unemp.rate.difference.edr.plot, width_svg = 10, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE) ,
                 opts_sizing(rescale = FALSE))
```
