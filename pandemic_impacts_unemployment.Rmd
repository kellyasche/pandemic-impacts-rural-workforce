...
One question is how much of the trend in Ui claims declining due to people dropping out of the workforce?

<br>

```{r prep ui laborforce decline pct ui decline, include=FALSE, cache=TRUE}
ui.monthly.change.edr <- ui.edr %>%
  gather(key = "date", value = "ui.claims", 3:(ncol(ui.edr)-1)) %>%
  mutate(ui.claims = as.integer(ui.claims),
         date = as_date(date, format = "%m/%d/%Y"),
         month = month(date)) %>%
  ...
  group_by(edr, month) %>%
  summarise(avg.ui.claims = round(mean(ui.claims, na.rm = TRUE))) %>%
  ungroup() %>%
  group_by(edr) %>%
  arrange(month) %>%
  mutate(avg.ui.claims.change = avg.ui.claims - lag(avg.ui.claims)) %>%
  ungroup()

laborforce.change.edr <- master.laborforce.edr %>%
  mutate(`Year/Month` = ym(`Year/Month`),
         month = month(`Year/Month`)) %>%
  group_by(edr) %>%
  arrange(month) %<>%
  mutate(labor.force.change = labor.force - lag(labor.force)) %>%
  ungroup()

ui.monthly.laborforce.change.edr <- ui.monthly.change.edr %>%
  left_join(laborforce.change.edr[,c(2,3,4,5)], by = c("edr", "month")) %>%
  mutate(change.ui.claims.pct.laborforce.change = avg.ui.claims.change / labor.force.change)

names(ui.monthly.laborforce.change.edr)
names(laborforce.change.edr)
```
